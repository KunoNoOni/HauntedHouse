package
{
	...
	public class Spider extends FlxSprite
	{
		...
		{
			super(X*16,Y*16);
			loadGraphic(Registry.Spider,true,false,16,16);
			addAnimation("crawl",[0,1],5,true);
			
			rnd = Math.floor(Math.random()*4)+1;
			floor = rnd;
			//trace("Spider is starting on floor "+this.floor);
		}
		...
		public function checkStairs():void
		{
			...
			//trace("Spider is now on floor "+this.floor);
		...
		public function setPath():void
		{
			...
			//trace("The spider is at "+Math.floor(this.x)/16+","+Math.floor(this.y)/16);
